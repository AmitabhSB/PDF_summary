...
#
# Parameters:
# 1. sourcePDFFile - Source PDF file 
...
if os.path.exists(sourcePDFFile):
    ...
    for p,t in sorted([(v,k) for k,v in pdfFileObj.getDestinationPageNumbers().items()]):
         ...
         #   print('First Page...')
          ...
          #  print('Created PDF file: ' + outputPDFDir + pdfFileName)
           ...
           # print('Created TXT file: ' + outputTXTDir + txtFileName)
            ## for plain text files find Summary and save in a text file
            parser = PlaintextParser.from_file(outputTXTDir + txtFileName, Tokenizer(LANGUAGE))
            stemmer = Stemmer(LANGUAGE)
            summarizer = LsaSummarizer(stemmer)
            summarizer.stop_words = get_stop_words(LANGUAGE)
            summaryOutputFile = open(outputSummaryDir + 'SummaryFile.txt','a')
      ...
      #  print('Created TXT file: ' + outputSummaryDir + 'SummaryFile.txt')
